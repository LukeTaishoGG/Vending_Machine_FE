;(() => {
  ...
  ;((a.id = 17),
    ...
    (a.modules = {
      ...
      2348: (a, b, c) => {
        Promise.resolve().then(c.bind(c, 8320))
      },
      3033: (a) => {
        'use strict'
                    ...
                    children: [
                      'add-pin',
                      {
                        children: [
                          ...
                          {
                            page: [
                              () => Promise.resolve().then(c.bind(c, 1804)),
                              '/Users/luke/LDH/Works/VendingMachine/Vending_Machine_FE/src/app/(app)/add-pin/page.tsx',
                            ],
                          },
                        ],
                      },
...
see more here https://nextjs.org/docs/messages/app-static-to-dynamic-error`),
      ...
      8320: (a, b, c) => {
        ...
        let d = (0, c(7954).registerClientReference)(
          function () {
            throw Error(
              'Attempted to call the default export of "/Users/luke/LDH/Works/VendingMachine/Vending_Machine_FE/src/views/AddPinPage.tsx" from the server, but it\'s on the client. It\'s not possible to invoke a client function from the server, it can only be rendered as a Component or passed to props of a Client Component.',
            )
          },
          '/Users/luke/LDH/Works/VendingMachine/Vending_Machine_FE/src/views/AddPinPage.tsx',
          'default',
        )
      },
      8354: (a) => {
      ...
      9196: (a, b, c) => {
        Promise.resolve().then(c.bind(c, 9929))
      },
      9294: (a) => {
        'use strict'
      ...
      9929: (a, b, c) => {
        ...
        var d = c(1124),
          ...
          l = c(5745)
        let m = async (a) => {
            let b = await fetch(`${l.H}/map_pins`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(a),
            })
            if (!b.ok) throw Error('ピンデータの保存に失敗しました')
            return b.json()
          },
          n = async (a) => {
            let b = await fetch(`${l.H}/categories`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ name: a }),
            })
            if (!b.ok) throw Error('カテゴリデータの保存に失敗しました')
            return b.json()
          },
          o = async (a) => {
            let b = await fetch(`${l.H}/manufacturers`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ name: a }),
            })
            if (!b.ok) throw Error('メーカーデータの保存に失敗しました')
            return b.json()
          },
          p = async (a) => {
            let b = await fetch(`${l.H}/price_ranges`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ price_range: a }),
            })
            if (!b.ok) throw Error('価格帯データの保存に失敗しました')
            return b.json()
          },
          q = async (a) => {
            let b = await fetch(`${l.H}/products`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ name: a }),
            })
            if (!b.ok) throw Error('商品データの保存に失敗しました')
            return b.json()
          },
          r = async (a) => {
            let b = await fetch(`${l.H}/machine_descriptions`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ description: a }),
            })
            if (!b.ok) throw Error('自販機説明データの保存に失敗しました')
            return b.json()
          },
          s = async (a) => {
            let b = await fetch(`${l.H}/vending_machines`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(a),
            })
            if (!b.ok) throw Error('自販機データの保存に失敗しました')
            return b.json()
          }
        var t = c(1428),
          u = c(6463),
            ...
            let a = (0, j.useRouter)(),
              { user: b } = (0, u.Jd)(),
              {
                ...
                let b = (0, t.O)(),
                  ...
                  [e, f] = (0, i.useState)(''),
                  [g, h] = (0, i.useState)(b),
                  [j, l] = (0, i.useState)(null),
                  [u, v] = (0, i.useState)(!1),
                  w = (0, i.useCallback)(async (a) => {
                    if (a.trim())
                      try {
                        let b = await (0, k.f)(a)
                        b && (h(b), l(b), d((a) => ({ ...a, ...b })))
                  ...
                  x = (0, i.useCallback)(async (a) => {
                    if (!a.latLng) return
                    let b = a.latLng.lat(),
                      c = a.latLng.lng()
                    ;(l({ lat: b, lng: c }), d((a) => ({ ...a, lat: b, lng: c })), v(!0))
                    try {
                      let a = await (0, k.R)(b, c)
                      a && d((b) => ({ ...b, address: a }))
                    } catch (a) {
                ...
                let y = async (b) => {
                  ...
                  try {
                    let b = ((a) => {
                        if (!a?.id) throw Error('ユーザーIDが取得できません')
                      ...
                      e = await n(c.category),
                      f = await o(c.manufacturer),
                      g = await p(c.priceRange),
                      h = await q(c.products),
                      i = await r(c.description)
                    return (
                      await s({
                        pin_id: d.id,
                        user_id: b,
                    ...
                    return (
                      console.error('ピン追加エラー:', a),
                      alert(a instanceof Error ? a.message : '登録失敗'),
                      { success: !1 }
                    )
                  }
                }
            ...
            children: (0, d.jsxs)(f.T, {
              ...
              children: [
                (0, d.jsx)(g.E, {
                  fontSize: '2xl',
                  fontWeight: 'bold',
                  children: '追加する自動販売機の情報を入力してください \uD83E\uDDD0',
                }),
                (0, d.jsxs)(e.a, {
                  children: [
                    ...
                    (0, d.jsx)(h.A, {
                      ...
                      onKeyPress: (a) => {
                        'Enter' === a.key && B(a.currentTarget.value)
                      },
                      placeholder: '住所を検索してマップを移動（Enterキーで検索）',
                    }),
                  ],
                }),
                (0, d.jsx)(w.y, { center: c, zoom: 15, selectedLocation: l, onMapClick: C }),
                (0, d.jsxs)(e.a, {
                  as: 'form',
                  onSubmit: E,
                  children: [
                    (0, d.jsx)(v.d, { formData: a, isLoadingAddress: y, onInputChange: D }),
                    (0, d.jsx)(x.E, { onSubmit: E, onCancel: F, isSubmitting: z }),
                  ],
                }),
              ],
            }),
  ...
  b.C(a)
  var c = b.X(0, [838, 901, 381], () => b((b.s = 5600)))
  module.exports = c
})()
