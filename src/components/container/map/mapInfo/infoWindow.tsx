...
const InfoWindowContent = ({ pinId }: InfoWindowContentProps) => {
  ...
  <div style={{ padding: '16px' }}>
    ...
    <div style={{ fontSize: '14px', lineHeight: '1.6' }}>
      ...
      <p><strong>自販機ID:</strong> {data.vending_machine_id}</p>
      <p><strong>緯度:</strong> {data.latitude}</p>
      <p><strong>経度:</strong> {data.longitude}</p>
      <p><strong>メーカー:</strong> {data.manufacturer_name || '未設定'}</p>
      <p><strong>カテゴリ:</strong> {data.category_name || '未設定'}</p>
      <p><strong>価格帯:</strong> {data.price_range || '未設定'}</p>
      <p><strong>商品:</strong> {data.product_name || '未設定'}</p>
      {data.description && (
